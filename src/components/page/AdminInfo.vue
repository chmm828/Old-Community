  ...
  <div>
    ...
    <el-card class="box-card">
      <el-table :data="userData" stripe style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="a_name" label="姓名" align="center" width="100"></el-table-column>
...
export default {
  data() {
    var handleFindAccount = (rule, value, callback) => {
      if (value === undefined || value === "") {
        ...
        const that = this
        that.form.a_id = sessionStorage.getItem('admin_id');
        that.$http
  ...
  methods: {
    findUsetInfo() {
      const adminInfo = {
        a_id: sessionStorage.getItem("admin_id")
      };
      this.loading = true;
      this.$http
        .post("/api/updatePassword/findAdminInfo", adminInfo)
        .then(res => {
    ...
    clickOk(e) {
      ...
      this.$refs.editUserDate.validate(valid => {
        ...
        this.$http.put("/api/admin/updateAdmin", values).then(res => {
          console.log(res);
          if (res.data === "更新成功") {
    ...
    handleUpdatePwd() {
      this.$refs.form.validate(valid => {
        ...
        this.$http
          .put("/api/updatePassword/updateAdminPassword", this.form)
          .then(res => {
