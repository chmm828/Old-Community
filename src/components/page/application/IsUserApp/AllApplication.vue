  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>全部的申请记录</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-card class="box-card">
      <el-row>
        <el-col :span="6">
          <el-input
            ...
            clearable
          >
            <el-button slot="append" icon="el-icon-search" @click="findAppIdIsAllPage" />
          </el-input>
        </el-col>
        <el-col :span="2">
        ...
        <el-col :push="1" :span="2">
          <el-button
            ...
            type="info"
            @click="findApplicationIsAlltPage"
          >刷新</el-button>
        </el-col>
      </el-row>
      <el-table :data="userData" stripe style="width: 100%" :loading="loading">
        ...
        <el-table-column
          ...
          label="申请状态"
          width="100"
          :filters="[{ text: '待审核', value: 0 }, { text: '通过', value: 1 }, { text: '不通过', value: 2 }, { text: '已取消', value: 3 }]"
        ...
        <el-table-column label="操作"  width="300" fixed="right">
          <template slot-scope="scope">
            ...
            <el-popconfirm
              ...
              title="确定取消吗？"
            >
              <el-button type="warning" slot="reference" size="mini">取消申请</el-button>
            </el-popconfirm>
            <!-- <el-popconfirm
              v-if="scope.row.app_status === 0"
              @confirm="isCancel(scope.row)"
              ...
              title="确定取消吗？"
            >
              <el-button type="warning" size="mini">取消申请</el-button>
            </el-popconfirm>-->

            <el-button
              type="primary"
              v-if="scope.row.app_status === 2 || scope.row.app_status === 3"
              size="mini"
              @click="handleInsert"
            >重新申请</el-button>
            <el-popconfirm
              v-if="scope.row.app_isDelete === 0 && scope.row.app_status !=0"
              confirm-button-text="好的"
              cancel-button-text="不用了"
...
export default {
  ...
  methods: {
    ...
    findAppIdIsAllPage() {
      ...
      this.loading = true;
      this.findAppIdIsAllTotal();
      this.$http
        .post("/api/userAppApi/findAppIdIsAllPage", this.queryInfo)
        .then(res => {
          this.userData = res.data || [];
          this.loading = false;
        });
    ...
    findAppIdIsAllTotal() {
      this.queryInfo.u_id = sessionStorage.getItem("user_id");
      this.$http
        .post("/api/userAppApi/findAppIdIsAllTotal", this.queryInfo)
        .then(res => {
    ...
    onSubmit(e) {
      this.$refs.editUserDate.validate(valid => {
        ...
        if (
          ...
          this.editUserDate.u_id = sessionStorage.getItem("user_id");
          this.$http
            .post("/api/userAppApi/inserApplicationInfon", this.editUserDate)
            .then(res => {
              // console.log(res);
          ...
          this.$http
            .put("/api/userAppApi/updateApplicationInfon", this.editUserDate)
            .then(res => {
              if (res.data.code === 200) {
    ...
    isCancel(row) {
      row.app_status = 3;
      this.$http
        .put("/api/userAppApi/updateApplicationInfon", row)
        .then(res => {
          if (res.data.code === 200) {
