  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>全部的事项</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <el-card class="box-card">
      ...
      <el-table :data="userData" stripe style="width: 100%" :loading="loading">
        ...
        <el-table-column
          ...
          label="记录状态"
          width="120"
          :filters="[{ text: '正常', value: 0 }, { text: '用户已删除', value: 1 }]"
          :filter-method="filterIsDelete"
          filter-placement="bottom-end"
        >
          <template slot-scope="scope">
            <el-tag v-if="scope.row.app_isDelete === 0" type="success" disable-transitions>正常</el-tag>
            <el-tag v-else type="danger" disable-transitions>用户已删除</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="300" fixed="right">
          <template slot-scope="scope">
            ...
            <el-popconfirm
              ...
              title="确定帮用户取消吗？"
            >
              <el-button type="warning" slot="reference" size="mini">帮用户取消</el-button>
            </el-popconfirm>
           </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    findApplicationIsAlltPage() {
      ...
      this.$http
        .post("/api/adminAppApi/findApplicationIsAlltPage", this.queryInfo)
        .then(res => {
          // console.log("====>", res);
    ...
    findApplicationIsAll() {
      ...
      this.$http
        .post("/api/adminAppApi/findApplicationIsAll", userInfo)
        .then(res => {
    ...
    findAppIdIsAllPage() {
      ...
      this.$http
        .post("/api/adminAppApi/findAppIdIsAllPage", this.queryInfo)
        .then(res => {
          this.userData = res.data || [];
          this.loading = false;
        });
    ...
    findAppIdIsAllTotal() {
      this.queryInfo.u_id = sessionStorage.getItem("user_id");
      this.$http
        .post("/api/adminAppApi/findAppIdIsAllTotal", this.queryInfo)
        .then(res => {
    ...
    // 属性即可开启该列的筛选，用来筛选
    filterIsDelete(value, row) {
      return row.app_isDelete === value;
    },
    // 查找未禁用的社区
    findHome() {
    ...
    isCancel(row) {
      row.app_status = 3;
      row.app_a_id = sessionStorage.getItem('admin_id')
