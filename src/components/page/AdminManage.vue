<template>
      ...
      <el-row>
        <el-col :span="6">
          <el-input
            ...
            clearable
          >
            <el-button slot="append" icon="el-icon-search" @click="findAdminName" />
          </el-input>
        </el-col>
        <el-col :span="2">
      ...
      <el-table :data="userData" stripe style="width: 100%" :loading="loading">
        ...
        <el-table-column label="头像">
          <template slot-scope="scope">
            <el-avatar
              :size="35"
              v-if="scope.row.admin_pic"
              :src=" `http://localhost:3000/${scope.row.admin_pic}` "
            ></el-avatar>
            <el-avatar
              :size="35"
              v-else
        ...
        <el-table-column prop="a_name" label="姓名" align="center" width="200"></el-table-column>
        <el-table-column prop="a_account" label="账号" width="180" align="center"></el-table-column>
        ...
        <el-table-column label="操作" align="center" width="300">
          1
          <template slot-scope="scope">
            <el-button size="mini" @click="handleEdit(scope.row)"  v-if="scope.row.role==='admin'">编辑</el-button>
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible">
      <el-form :model="editUserDate" ref="editUserDate" :rules="rules">
        ...
        <el-form-item label="账号" :label-width="formLabelWidth" prop="a_account">
          <el-input v-model="editUserDate.a_account" disabled></el-input>
        </el-form-item>
  ...
  <script>
import { flat } from "shelljs/commands";
...
export default {
  ...
  methods: {
    ...
    findUsetInfo() {
      this.loading = true;
      this.findUsetTotal();
      this.$http.post("/api/admin/getAdmin", this.queryInfo).then(res => {
        this.userData = res.data;
        this.loading = false;
      });
    },
    findUsetTotal() {
      this.$http.get("/api/admin/getAdminTotal").then(res => {
        this.total = res.data.length || 0;
      });
    },
    findAdminName() {
      this.loading = true;
      this.findAdminNameTotal();
      this.$http
        .post("/api/admin/findAdminNamePage", this.queryInfo)
        .then(res => {
          this.userData = res.data || [];
          this.loading = false;
        });
    ...
    findAdminNameTotal() {
      this.$http.post("/api/admin/findAdminNameTotal" , this.queryInfo).then(res => {
    ...
    clickOk(e) {
      this.$refs.editUserDate.validate(valid => {
        if (!valid) return this.$message.error("表单校验不通过，请检查后提交");
        this.$http
          .put("/api/admin/updateAdmin", this.editUserDate)
          .then(res => {
            // console.log(res);
            if (res.data === "更新成功") {
              this.$message.success(res.data);
